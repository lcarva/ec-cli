...
// setExpiration sets the "expires_on" attribute on records. The expires_on value for record n is
// the effective_on value of the n-1 record. The first record on the list does not contain an
// expires_on value since there is no newer record that invalidates it in the future.
...
// period.
// TODO: The pruning function above could be simplified to just delete records that have an
// expires_on data in the past.
